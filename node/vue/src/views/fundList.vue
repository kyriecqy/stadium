...
<!--
  ...
  <div>
    <div class="btn">
      ...
      <el-form label="按照时间筛选:" class="date_picker" slot-scope="">
        <el-date-picker
          v-model="date_picker"
          type="datetimerange"
          ...
          size="small"
          @change="getDate_picker"
        />
      </el-form>
      <button class="btn_item_add" @click="add" v-if="user_identify == '管理员'">添加</button>
    </div>
    <div class="table">
...
export default ({
  ...
  computed: {
    user_identify() {
      return store.getters.user.identify
    }
  },
  methods: {
    //从数据库去具体数据
     async getProfile(){
      await axios.get('http://localhost:4000/profiles').then(res => {
        ...
        this.date_picker_tableData = res.data
    ...
    //筛选之后显示的数据
    getDate_picker(date) {
      //let myDate = new Date('2022-5-4')
      //console.log(date);
      this.page_tableData = this.date_picker_tableData.filter((item,index) => {
        let myDate = new Date(item.date)
        return myDate >= date[0] && myDate <= date[1]
      })
      this.paginations.total = this.page_tableData.length
    }
  },
  mounted() {
    this.getProfile()
